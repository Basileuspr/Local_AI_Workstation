  ...
  return (
    <section id="lora-studio">
      <header className="lora-header">
        ...
        <div className="lora-project-picker"><select aria-label="LoRA project" value={project.id} onChange={(event) => loadProject(event.target.value).catch(error => toast(error.message, "error"))} disabled={loading || trainingSubmitting || imageBatch.busy}>{projects.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.image_count})</option>)}</select><button type="button" onClick={openCreateProject} disabled={loading || trainingSubmitting || imageBatch.busy}>+ New</button><LoraHelp /></div>
      </header>
      <div className="lora-layout">
        ...
        <div className="lora-column">
          <section className="lora-card">
            ...
            <div className="lora-form-grid">{fields.map(([key, label, type, min, max, step]) => <label key={key}>{label}<input type={type} min={min} max={max} step={step} value={settings[key] ?? ""} disabled={workspaceBusy} onChange={(event) => editSettings({ [key]: Number(event.target.value) })} /></label>)}<label>Precision<select value={settings.precision} disabled={workspaceBusy} onChange={(event) => editSettings({ precision: event.target.value })}><option value="fp16">FP16</option><option value="bf16">BF16</option><option value="fp32">FP32</option></select></label><label>Aspect handling<select value={settings.aspect_mode} disabled={workspaceBusy} onChange={(event) => editSettings({ aspect_mode: event.target.value })}><option value="crop">Center crop</option><option value="pad">Pad to fit</option></select></label></div>
