...
def read_gpus():
    fields = "index,name,utilization.gpu,temperature.gpu,clocks.current.graphics,clocks.current.memory,memory.total,memory.used,power.draw"
    try:
...
$ram = @(Get-CimInstance Win32_PhysicalMemory | Select-Object DeviceLocator,ConfiguredClockSpeed)
$sensors = @()
...
@{ram=$ram; sensors=$sensors} | ConvertTo-Json -Depth 4 -Compress
"""
...
def read_hardware():
    if os.name == "nt":
        try:
            data = json.loads(run_command(["powershell.exe", "-NoProfile", "-NonInteractive", "-Command", WINDOWS_SENSORS]))
            temperatures = [
            ...
            ram = [{"slot": entry.get("DeviceLocator") or "Memory module", "speed_mts": number(entry.get("ConfiguredClockSpeed")) or None} for entry in data.get("ram", [])]
            return temperatures, ram
        except (OSError, subprocess.SubprocessError, ValueError, TypeError, KeyError):
            return [], []
    try:
        sensors = psutil.sensors_temperatures()
        temperatures = [{"name": item.label or group, "temperature_c": number(item.current)} for group, items in sensors.items() if group.lower() in ("coretemp", "k10temp", "cpu_thermal", "zenpower") for item in items]
        return temperatures, []
...
class StatsSampler:
    ...
    def snapshot(self):
        ...
        with self.lock:
            ...
            temperatures, ram_modules = self.hardware
            errors = []
            try:
