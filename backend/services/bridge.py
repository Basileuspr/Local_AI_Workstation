...
async def peer_request(peer, method, path, body=None):
    ...
    async with httpx.AsyncClient(verify=tls_context(peer["cert"]), trust_env=False, timeout=30) as client:
...
class Bridge:
        ...
        with self.lock:
            ...
            try:
                ...
                config = uvicorn.Config(self.gateway, log_config=None, log_level=None,
                                        http=QuietBridgeProtocol, ws="none",
    ...
    def status(self):
        return {"running": self.running, "url": self.url, "name": self.name, "id": self.node_id,
    ...
    async def capabilities(self):
        ...
        models, images = await asyncio.gather(self.local_get("/models"), self.local_get("/image-generation/models"))
        gpus, _ = await run_in_threadpool(read_gpus)
        return {"protocol": PROTOCOL, "name": self.name, "host": host_resources(),
                ...
                "chat_error": models.get("error"), "image_error": images.get("runtime", {}).get("error"),
    ...
    async def sync(self, job_id, submit=False):
        ...
        try:
            ...
            return self.store.update("outgoing", job_id, connection_error="Connection or peer response unavailable. Execution status is unknown; reconnect to retrieve it. Retry delivery uses the same job ID.")
        finally:
            with self.lock:
                ...
                if current["cancel_requested"] and queued:
                    ...
                    async with httpx.AsyncClient(trust_env=False, timeout=5, transport=self.local_transport) as client:
                        with suppress(httpx.HTTPError):
                            await client.post(self.local_url + stop_path + request_id, headers=headers)
